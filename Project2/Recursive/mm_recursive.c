...
void RecMult(int n, matrix a, matrix b, matrix c)
{
	...
	if (n <= block) {
		...
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) 
				for (k = 0; k < n; k+=4)
				  	r[i][j] += p[i][k] * q[k][j] + p[i][k+1]*q[k+1][j] +p[i][k+2]*q[k+2][j] +p[i][k+3]*q[k+3][j] ;
		}
    	} 
	else {
		d=newmatrix(n);
		n /= 2;
		cilk_spawn RecMult(n, a11, b11, d11);
		cilk_spawn RecMult(n, a12, b21, c11);
	
		cilk_spawn RecMult(n, a11, b12, d12);
		cilk_spawn RecMult(n, a12, b22, c12);

		cilk_spawn RecMult(n, a21, b11, d21);
		cilk_spawn RecMult(n, a22, b21, c21);

		cilk_spawn RecMult(n, a21, b12, d22);
		RecMult(n, a22, b22, c22);
		
		cilk_sync;	
		cilk_spawn RecAdd(n, d11, c11, c11);
		cilk_spawn RecAdd(n, d12, c12, c12);	
		cilk_spawn RecAdd(n, d21, c21, c21);		
